...
measures of prediction performance \cite{craddock2009, deshpande} or a measure
of their stability \cite{craddock2009}.  A
third approach are to use sparse modeling methods such as L1 SVM or LASSO that
force weights for non-essential features to be zero [tibs,??]. 
No matter
...
The interpretation of connections implicated by MPM and their relationship
with a phenotypic outcome is complicated and requires insight into the
mechanisms underlying the modeling approach. If features are appropriately
standardized prior to training, the magnitude of model weights can be
interpreted to reflect the relative importance of the corresponding feature to the model,
similar to beta values from a standard regression model. But due to the
multivariate nature of the MPMs, the sign of the weight cannot necessarily be
interpreted to indicate the relationship between a connection and a phenotypic
variable. Indeed, it is possible for the sign of a model weight to change based
on the inclusion or exclusion of another feature in the model
\cite{craddock2009}.  It is perhaps

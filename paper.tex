...
it incorporates the largest amount of data into training performs the largest amount of data as it is
easy to implement and results in the same error estimate, its estimates are more variable than 
kfcv or bscv. Split-half cross validation (two-fold cross validation, 2fcv), minimizes the estimation
variance, at the cost of using less training


single observation (leave-one-out cross-validation; loocv), a fraction of the observations (k-fold cross-validation; kfcv),
or resampling the observations with replacement (bootstrap cross-validation; bscv);  


, leaving a  A commmon practice is leave-on-out cross-validation (loocv) in which a single observation
is used for testing at each iteration, 


is estimated through a one or
more iterations of cross validation. 


A cross validation cycle begins by seperating the 
available data into two independent subsets: training data and testing data. A predictive model 
is learned from the training data and subsequently applied to the testing data to predict 
corresponding values of the target variable. Prediction accuracy is then assessed by comparing
these predictions to the observed value of the target variable. This procedure is typically repeated
for several different splits of the data. The simplest is hold-out validation in which the data
is split into testing and training 
	
	One of the most important parameters for cross validation
is the manner in which data are subdivided, which sets the balance between maximally use of the
data for training.
	
	
	Cross-validation can be performed
in a variaty of ways, and if performed correctly will result in an unbiased estimator of 
generalization ability. A major choice 

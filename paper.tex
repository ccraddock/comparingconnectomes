...
% Define terms
\hl{``Functional connectivity'' is defined as a measure of synchronization in 
brain signals \cite{friston1994}. More generally, it is interesting as a 
window on underlying synchrony in neural processes \cite{lee2003}.}
...
spatial maps
that capture functional networks or confounding effects, \hl{as for instance
...
hand. \hl{Finally, it is important to keep in mind that there is no
...
\hl{Studying connectivity from \emph{evoked} activity with task-driven studies 
it requires disambiguating task-specific connectivity effects from
...
matrix \hl{(also called \emph{precision matrix})} captures
...
that lies in a network of strongly connected nodes, a \hl{large
sub-network} is
...
measures of connectivity \hl{\cite{richiardi2011,dosenbach2010,shirer2012}}, or any of the previously discussed graph
summary metrics \hl{\cite{cecchi2009,ekman2012}}.
%
...
are useful''. Depending on the question and the data at hand, \hl{a trade-off 
...
comparison, which will give \hl{well-controlled} results only if the true generative
...
self-consistence, \hl{as the Neyman-Pearson lemma is established under the
...
question. \hl{Similarly, the idealized notion of an unique \emph{functional
...
for their suggestions that improved the manuscript.

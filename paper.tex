...
\cite{friston1994, biswal1995}.  Given that
...
coherent patchs of cortex whose activity is synchronized through
cytoarchitecture and edges describe large range synchronizations between nodes
...
\cite{satterthwaite2012, saad2012}.
...
% CAMERON I think that this reference to Fair should be Dosenbach 2006 neuron
...
choices are the the Automatic Anatomic Labeling (AAL) atlas \cite{tzourio-mazoyer2002a}, 
which benefits from an SPM toolbox or the ubiquitous Talaraich-Tournoux
atlas \cite{talaraich}. 
...
they focus on labeling large anatomical structures and does not match
...
% CAMERON include citation to Dosenbach, and some others
\cite{dosenbach2006,grillon2012}. The common practice is to place balls of a given
...
\cite{craddock2011, germanguy}. In general these metrics do not result in an
...
% CAMERON = using CompCor in addition to WM and CSF is redundant as 
% CompCor involves a PCA decomposition of WM and CSF
...
rest data, signal extraction can be acheived by either averaging the fmri time
...
principle components analysis of the time series \cite{Friston functional
	localizers}.  The latter approach provides an additional level of
denoising by selecting a time series that explains the greatest variance across
the voxels, and excludes ``deviant'' time series from noise or dissimilar
voxels. Comparisons of these methods has shown that the eigenvariate method is
more sensitive to function inhomogeniety \cite{craddock 2011} and exhibits worse
test-retest reliablity than averaging time series \cite{zuo thingy}. A hybrid
approach would be to average the top few eigenvariates from the PCA
decomposition.  In addition, improved specificity to BOLD signal can be enforced
...
% CAMERON make this better
Estimating intrinsic activity from task based data can be obtained by either
temporally concatenating rest blocks from the task, or by regressing the task
from the fMRI data. The latter approach is justified by the observation that
spontaneous activity is present during and additive to task BOLD responses 
\cite{fox finger tapping}. Evaluations of these methods have shown variable
results, where the resulting connectivity is similar too, but not the same as
results obtained from resting state data \cite{fair, lohman}. The differences 
are likely due to the
large carry over effects seen from task to rest blocks. 
...
%
% CAMERON might want to pull this back to graph edges
...
[kriegeskorte, Kjems].  When combined with reproducibility, prediction accuracy
provides a metric for evaluating experimental tradeoffs for data acquisition,
preprocessing, and analysis [Strother, LaConte].  Multivariate predictive models
...
test. From the perspective of medical applications, using resting-state
connectivity as a biomarker is particularly appealing as it can be applied to
impaired patients \cite{greicius2008b}. Predictive modeling techniques have been
successfully applied to identify connectome-based biomarkers of Alzheimer's
disease \cite{stonnington2010}, depression \cite{craddock2009}, schizophrenia
...
% interest/need for predictive modeling in connectomics as well as the number of
% approaches applicable 
Recent work has illustrated the utility of predictive modeling for deriving
connectivity models at the individual level [Chu 2010, Craddock in press or
HBM].

...
\begin{abstract}
	...
	the brain. With task-drive experiments they represent integration
	mechanisms between specialized brain areas. Analyzing the variability in
	the functional connectome across subjects and conditions can reveal
	...
	functional-connectivity methods and expose different steps in performing
...
smoothing), connectivity based analyses require additional denoising 
...
full-brain parcellations \cite{craddock2012, bellec2010, yeo2011,
...
structures \cite{craddock2012, varoquaux2012} (see fig.\,\ref{fig:parcellations}).
...
is attractive in connectomics because it is sensitive to dependencies
between features and avoids the need to correct for multiple comparisons since
the significance of an entire pattern is evaluated using a single statistical
test. 
Additionally many MPM techniques exist which are specifically
...
performance. Several strategies exist for performing cross-validation and the commonly
...
\cite{hastie2003}.
Alternative approaches such as (5 or 10)-fold cross-validation, split half
resampling, or $0.632+$ bootstrap should be preferred \cite{hastie2003}. The significance of
...
information exists in the connectome about a phenotypic variable, identifying
the connections most relevant to the predictive model (feature selection) is
less straight forward.  These problems can be somewhat mitigated by carefully
previously described sparse inverse covariance estimation techniques to minimize
the number of connections. Feature selection \cite{guyon2003} can be performed by filtering out
features based on their statistical relationship with the variable of interest
\cite{craddock2009, shen2010},
selecting features based on cross-validation \cite{craddock2009, deshpande2010}, or by employing sparse modeling
methods that force weights for non-essential features to be zero
\cite{tibshirani1994, michel2009, ryali2010}.
Filtering features based on a univariate criterion such as t-test should be
avoided, since they are insensitive to multivariate information and may exclude
features that could be important to a multivariate model \cite{guyon2003,
	craddock2009}. No matter which
feature selection strategy is employed, it must be performed within
cross-validation to avoid biasing estimates of prediction accuracy.
 
The interpretation of connections implicated by MPM and their relationship with
a phenotypic outcome is complicated and requires insight into the mechanisms
underlying the modeling approach. If features are appropriately standardized
prior to training, the magnitude of model weights can be interpreted to reflect
the relative importance of the corresponding feature to the model, similar to
beta values from a standard regression model. But due to the multivariate nature
of the MPMs, the sign of the weight cannot necessarily be interpreted to
indicate the relationship between a connection and a phenotypic variable.
Indeed, it is possible for the sign of a model weight to change based on the
inclusion or exclusion of another feature in the model \cite{craddock2009}.  It
